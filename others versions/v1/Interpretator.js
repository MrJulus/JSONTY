...
class JSONLangInterpreter {
    ...
    evaluateExpression(expression) {
        ...
        let evaluated = expression.replace(/\((.*?)\)/g, (_, variable) => {
            return this.variables[variable.trim()] ?? variable.trim();
        });
        ...
        for (let key in command) {
            if (key === "set") {
                ...
                if (fct) {
                    ...
                    if (fct.return) {
                        this.variables[fct.return] = interpreter.variables[fct.return];
                        this.variables["result"] = interpreter.variables[fct.return];
                    } else {
