        ...
        for (let key in command) {
            if (key === "set") {
                for (let varName in command[key]) {
                    let value = command[key][varName];
    
                    if (typeof value === "object" && value !== null) {
                        for (let type in value) {
                            if (["str", "int", "float", "bool"].includes(type)) {
                                this.variables[varName] = this.convert(value[type], type);
                            }
                        }
                    } else {
                ...
                let output = command[key].map(msg => {
                    if (typeof msg === "string") {
                        return msg.replace(/\((.*?)\)/g, (_, varName) => {
                            return this.variables[varName] ?? varName;
                        });
                    }
                    return msg;
                }).join("\n");
                ...
                console.log(output);
                    ...
                    let switchValue = this.evaluateExpression(command[key].value);
                    let matched = false;
            
                    switchValue = Number(switchValue);
                
                    for (let caseKey in command[key]) {
                        if (caseKey !== "value" && caseKey !== "default") {
                            if (switchValue === Number(caseKey)) {
                                this.executeCommand(command[key][caseKey].do);
                                matched = true;
                                break;
                            }
                        }
                    ...
                    if (!matched && command[key].default) {
                        this.executeCommand(command[key].default.do);
                    }                
            } else if (key === "if") {
